...
// Query: *[    _type == "contexto"    && (!defined($q) || titulo match $q + "*")    && (count($tagIds) == 0 || count((tags[]->_id)[@ in $tagIds]) > 0)  ] {    _id,    titulo,    slug,    fecha,    fechaFinal,    tiposDeContexto,    produccion,    tags[]->{_id, tag},  }
...
// Query: *[    _type == "programa"    && (!defined($q) || titulo match $q + "*")    && (count($tagIds) == 0 || count((tags[]->_id)[@ in $tagIds]) > 0)  ] {    _id,    titulo,    slug,    contexto,    fecha,    descripcionCorta,    coproduccion,    tags[]->{_id, tag},  }
...
// Query: {  "items": *[    _type == "transmision"    && (!defined($q) || titulo match $q + "*" || programa->titulo match $q + "*")    && (count($tagIds) == 0 || count((tags[]->_id)[@ in $tagIds]) > 0)    && (!defined($cursor) || fecha < $cursor)  ] | order(fecha desc) [0...$limit] {    _id, titulo, fecha, audio,    programa->{titulo},    tags[]->{_id, tag},    tipoDeTransmision[]->{_id, tipoDeTransmision}  },  "total": count(*[    _type == "transmision"    && (!defined($q) || titulo match $q + "*" || programa->titulo match $q + "*")    && (count($tagIds) == 0 || count((tags[]->_id)[@ in $tagIds]) > 0)  ])}
